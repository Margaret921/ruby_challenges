def add_up_birth_number(birthdate)
	number = birthdate[0].to_i + birthdate[1].to_i + birthdate[2].to_i + birthdate[3].to_i + birthdate[4].to_i + birthdate[5].to_i + birthdate[6].to_i + birthdate[7].to_i
end

def reduce_num(number)
	number = number.to_s
number = number[0].to_i + number[1].to_i + number[2].to_i
if (number > 9)
number.to_s
...
def get_birth_number(birthdate)
	number = add_up_birth_number(birthdate)
...
puts "What is your birthdate? Please supply in MMDDYYYY form!"
birthdate = gets
...
puts "You are a #{birth_path_number.to_s}. #{message}"
